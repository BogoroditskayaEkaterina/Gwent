...
def show_table(dbname):
    string = 'select * from ' + dbname + ' ORDER by name'
...
query7 = '''
DROP FUNCTION IF EXISTS clean_all_tables();
CREATE FUNCTION clean_all_tables()
RETURNS void
AS
$$
DELETE FROM cards;
...
def delete_card():
    cursor.execute(func.clean_all_tables())
...
CREATE FUNCTION delete_some_card(del_power bigint)
RETURNS void
AS
$$
...
def delete_some_cards(power):
    cursor.execute(func.delete_some_card(power))
