...
from db import search_ability, show_table, insert_card, delete_card, clean_table, delete_base, change_card, delete_some_cards, \
    create_db, load_triggers_index, insert_data_t1, insert_data_t2, show_base, close_base, connect_postgres, insert_data_t3, delete_chosen_card
...
def btn2():
    ...
    label = Label(window, text="Введите id карты")
    label.pack()
...
def btn9():
    window = Toplevel()
    window.geometry("600x250+400+400")
    label = Label(window, text="Введите новое название карты и старое")
    label.pack()
    entry = Entry(window, width=80)
    entry.pack()
...
def btn13():
    window = Toplevel()
    window.geometry("600x250+400+400")
    label = Label(window, text="Введите часть слова или слово")
    label.pack()
    entry = Entry(window, width=40)
    entry.pack()
